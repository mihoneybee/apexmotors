...
/* ==========================================================
   LOGICA DO MENU HAMBÚRGUER E SUPORTE A CLIQUES MOBILE
   ========================================================== */
document.addEventListener('DOMContentLoaded', () => {
    const nav = document.getElementById('navbar');
    
    // 1. Injeta o HTML do botão hambúrguer dinamicamente se não existir
    if (nav && !document.getElementById('hamburger')) {
        nav.insertAdjacentHTML('beforeend', `
            <div class="hamburger" id="hamburger">
                <span></span>
    ...
    // 2. Evento para abrir e fechar o menu completo
    ...
    // 3. Permite expandir as abas "Categorias" e "Marcas" ao tocar no Mobile
    ...
    dropdowns.forEach(dropdown => {
        ...
        if (dropbtn) {
            dropbtn.addEventListener('click', (e) => {
                if (window.innerWidth <= 992) {
                    e.preventDefault(); // Impede o link de navegar/recarregar
                    e.stopPropagation();
                    
                    // Fecha outros dropdowns abertos para não embolar
                    dropdowns.forEach(d => {
    ...
    // 4. Fecha o menu automaticamente ao clicar em um link comum (ex: Serviços, Quem somos)
    const linksNormais = document.querySelectorAll('.navbar-center .nav-links a:not(.dropbtn)');
